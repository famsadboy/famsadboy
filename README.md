...
_G.Color = Color3.fromRGB(0, 255, 0) ---Color

local Star = StarHub:Window("Xwift - Free Scripts")

local StarServer = Star:Server("Xwift HUB",7040391851)

local AutoFarm = StarServer:Channel("Auto Farm")



Client = AutoFarm:Label("")
...
local SelectWeapon = AutoFarm:Dropdown("Select Weapon Farm!",Wapon,function(Value)
...
AutoFarm:Toggle("Start Level-Farm", _G.AutoFarmLevel, function(vu)
    _G.AutoFarm = vu
	if _G.AutoFarm and SelectToolWeapon == "" then
		StarHub:Notification("AutoFarm","SelectWeapon First","Okay")
	else
		Auto_Farm = vu
		SelectMonster = ""
		if vu == false then
...
Y = 20
AutoFarm:Slider("Farming Distance", 0,50,Y, function(vu)
	Y = vu
end)

AutoFarm:Line()

AutoFarm:Toggle("Auto Make SuperHuman", _G.AutoSuperhuman, function(vu)
    Superhuman = vu
end)

AutoFarm:Toggle("Auto Fully Make Superhuman", _G.FullyAutoSuperhuman, function(vu)
...
AutoFarm:Toggle("Auto SetSpawn-Points", false, function(vu)
    AutoSetSpawn = vu
end)

if New_World then
    AutoFarm:Toggle("Auto Factory-Farm", false, function(vu)
        Factory = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
if Three_World then
	...
	AutoFarm:Toggle("Auto Enma/Yama-Sword", _G.AutoYama, function(vu)
		AutoYama = vu
	end)
...
if Old_World then
	AutoFarm:Toggle("Auto Second World", _G.AutoNewworld, function(vu)
		Auto_Newworld = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
AutoFarm:Toggle("Auto Make Electric-Claw", false, function(vu)
    AutoElectricClaw = vu
...
AutoFarm:Toggle("Auto Farm Mastery Devil Fruit", false, function(vu)
	...
	if AutoFarmMasteryFruit and WeaponMastery == "" then
		StarHub:Notification("Auto Farm Mastery","SelectWeapon First","OK")
	else
		FarmMasteryFruit = vu
		if vu == false then
			wait(1)
...
AutoFarm:Toggle("Auto Farm Mastery Gun", false, function(vu)
	...
	if AutoarmMasteryGun and WeaponMastery == "" then
		StarHub:Notification("Auto Farm Mastery","SelectWeapon First","Okay")
	else
		FarmMasteryGun = vu
		if vu == false then
			wait(1)
...
if New_World then
	AutoFarm:Toggle("Auto Quest Bartilo", false, function(vu)
		...
		if Success and CheckBarto == true then
			StarHub:Notification("Auto Quest Bartilo","Successfully","OK")
		elseif CheckBarto == true and MiscFarmWeapon == "" then
			StarHub:Notification("Auto Quest Bartilo","SelectWeapon First","OK")
	...
	AutoFarm:Toggle("Auto Evo Race", false, function(vu)
		...
		if not game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 3 and CheckEvo then
			StarHub:Notification("Auto Evo Race","You Have To Success Bartilo Quest","Okay")
		elseif CheckEvo == true and MiscFarmWeapon == "" then
			StarHub:Notification("Auto Evo Race","SelectWeapon First","Okay")
		elseif game:GetService("Players").LocalPlayer.Data.Race:FindFirstChild("Evolved") and CheckEvo then
			StarHub:Notification("Auto Evo Race","Successfully","Ok")
		else
			AutoEvoRace = vu
			if vu == false then
				wait(1)
...
if Three_World then
	AutoFarm:Toggle("Auto Citizen Quest", false, function(vu)
		...
		if CTCH and CheckCitizen then
			StarHub:Notification("Auto Quest Citizen","Successfully","Ok")
		elseif CheckCitizen and MiscFarmWeapon == "" then
			StarHub:Notification("Auto Quest Citizen","SelectWeapon First","Okay")
...
Stats:Toggle("Kaitan Stats", _G.Kaitan_Bypass, function(vu)
    Mad = vu
end)
...
local Raid = StarServer:Channel("Dungeon-Esp")
...
Raid:Toggle("Auto Awaken Skill", false, function(vu)
    AutoAwakener = vu
end)
...
Devil:Toggle("Buy Devil Fruit Sniper",false,function(vu)
    BuyFruitSinper = vu
end)
spawn(function()
    while wait(.1) do
...
Devil:Toggle("Auto Keep Fruits To Store", false, function(vu)
	AutoStoreFruit = vu
end)
...
Devil:Toggle("Bring Fruit - Risk", _G.BringFruit, function(value)
	BringFruit = value
end)
...
BuyItem:Toggle("Auto Buy All-Abilities", false, function(t)
    Abilities = t
...
BuyItem:Button("Refund Stat - 2500 Fragment", function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BlackbeardReward","Refund","1")
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BlackbeardReward","Refund","2")
end)

BuyItem:Button("Reroll Race - 3000 Fragment", function()
...
Setting:Button("Delete/Destroy UI", function()
	if game:GetService("CoreGui"):FindFirstChild("_G.StarHub .."," .. _G.Version") then
		game:GetService("CoreGui"):FindFirstChild("_G.StarHub .."," .. _G.Version"):Destroy()
	end
end)
...
Setting:Toggle("Auto Active Buso Haki", true, function(vu)
    Auto_Haki = vu
end)

Setting:Toggle("Auto Active Observation Haki", false, function(vu)
	AutoObservation = vu
end)

Setting:Label("Farming Mastery Settings")

Setting:Toggle("Auto : Z Skill", true, function(vu)
    SkillZ = vu
end)

Setting:Toggle("Auto : X Skill", true, function(vu)
    SkillX = vu
end)

Setting:Toggle("Auto : C Skill", true, function(vu)
    SkillC = vu
end)

Setting:Toggle("Auto :  V Skill", true, function(vu)
    SkillV = vu
end)
...
Setting:Button("BeautifulMode", function()
	BeautifulMode()
end)
...
local Config = StarServer:Channel("Config")

Config:Label("Xwift  | C O N F I G")

Config:Line()

Config:Label("_G.AutoFarmLevel = true")
Config:Label("_G.Auto_Elite_Hop  = true")
Config:Label("_G.AutoElite = true")
Config:Label("_G.AutoYama = true")
Config:Label("_G.AutoNewworld = true")
Config:Label("_G.AutoThirdSea = true")
Config:Label("_G.AutoSuperhuman = true")
Config:Label("_G.FullyAutoSuperhuman = true")
Config:Label("_G.Auto_Legendary_Sword = true")
Config:Label("_G.AutoLegendary_Hop = true")
Config:Label("_G.AutoEnchancement_Hop = true")
Config:Label("_G.AutoEnchancement = true")
